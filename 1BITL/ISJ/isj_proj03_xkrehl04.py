...
def match_permutations(string, words):
    ...
    matching_perms = sorted(var for var in words if var in perms) # vase reseni
...
def plur2sing(singular, plural):
    ...
    # inversion using zip
    pl2sg = dict((p,s) for s, p in sg2pl.items()) # vase reseni
    return pl2sg

def vect2word(word2vect):
    v2w = {tuple (v): k for v, k in zip(word2vect.values (), word2vect.keys ())} # vase reseni
    return v2w
